...
const ConnectionStatus = () => {
  ...
  useEffect(() => {
    const checkConnection = async () => {
      try {
        ...
        const response = await fetch(`${APP_CONFIG.API_BASE_URL}/utils/health`, {
          method: "GET",
          headers: {
  ...
  return (
    <div className="flex items-center" title={isConnected ? "Connesso al server" : "Server non raggiungibile"}>
      {isConnected ? <Wifi className="h-3.5 w-3.5 text-green-500" /> : <WifiOff className="h-3.5 w-3.5 text-red-500" />}
      <span className="ml-1 text-xxs text-gray-500">{isConnected ? "Connesso" : "Disconnesso"}</span>
    </div>
  );
};
